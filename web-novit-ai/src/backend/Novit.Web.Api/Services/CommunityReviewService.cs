    ...
    ILogger<CommunityReviewService> logger)
{
    private static readonly string[] ApproveKeywords =
        ["aprobar", "aprobado", "ok para publicar", "publicar", "publicalo", "publíquenlo", "publiquenlo"];

    private static readonly string[] RejectKeywords =
        ["rechazar", "rechazado", "descartar", "no publicar", "no postear", "cancelar publicacion", "cancelar publicación"];
    ...
    public string BuildReviewSubject(CommunityPublicationEntity draft) =>
        $"[REVISIÓN CM {draft.ReviewToken} v{draft.Version}] {draft.Topic}";
    ...
    public string BuildReviewEmailHtml(CommunityPublicationEntity draft)
    {
        ...
        var stageLabel = reviewStage == CommunityReviewStage.PreMedia
            ? "Aprobación previa a multimedia"
            : "Aprobación final";
        ...
        body.AppendLine("<p style=\"margin:0 0 10px 0;\"><b>Revisión de publicación social</b></p>");
        ...
        if (draft.ContentType.Equals("video_post", StringComparison.OrdinalIgnoreCase))
            body.AppendLine($"<p style=\"margin:0 0 8px 0;\"><b>Duración:</b> {draft.VideoDurationSeconds} segundos</p>");

        if (reviewStage == CommunityReviewStage.PreMedia)
            body.AppendLine("<p style=\"margin:12px 0 0 0;color:#5b5247;\">Esta es la validación previa al gasto fuerte: todavía no se generaron imágenes o videos finales. Acá se aprueban el copy y la dirección visual detallada.</p>");
        else
            body.AppendLine("<p style=\"margin:12px 0 0 0;color:#5b5247;\">Esta es la aprobación final antes de publicación. Ya incluye la multimedia generada lista para revisar.</p>");
        ...
        if (reviewStage == CommunityReviewStage.Final && mediaUrls.Count > 0)
        {
            ...
            body.AppendLine("<p style=\"margin:16px 0 8px 0;\"><b>Acciones rápidas:</b></p>");
        ...
        body.AppendLine("<li>Respondé con <b>APROBAR</b> para publicar.</li>");
        body.AppendLine("<li>Respondé con <b>RECHAZAR</b> para descartar esta pieza.</li>");
        body.AppendLine("<li>Si querés cambios, respondé con el feedback y, si podés, empezá el mail con <b>TEXTO:</b>, <b>VIDEO:</b> o <b>TODO:</b>.</li>");
        body.AppendLine("</ul>");
        body.AppendLine("</div>");
    ...
    public static string DetectRevisionTarget(string body)
    {
        ...
        if (normalized.StartsWith("video:") || normalized.StartsWith("imagen:") || normalized.StartsWith("diseño:"))
        ...
        var mentionsDesign = normalized.Contains("video") || normalized.Contains("reel") || normalized.Contains("duración") || normalized.Contains("duracion") || normalized.Contains("imagen") || normalized.Contains("visual") || normalized.Contains("diseño") || normalized.Contains("diseno");
        var mentionsCopy = normalized.Contains("texto") || normalized.Contains("copy") || normalized.Contains("caption") || normalized.Contains("gancho") || normalized.Contains("hashtag") || normalized.Contains("titulo") || normalized.Contains("título");
    ...
    private static string BuildVideoPlanHtml(JsonElement root)
    {
        ...
        if (!string.IsNullOrWhiteSpace(avatarScript))
            body.AppendLine($"<div style=\"white-space:pre-wrap;background:#fff;padding:12px;border:1px solid #ddd;border-radius:8px;margin:0 0 12px 0;\"><b>Locución / guion:</b><br />{WebUtility.HtmlEncode(avatarScript)}</div>");
        ...
        if (video.TryGetProperty("supporting_clips", out var clips) && clips.ValueKind == JsonValueKind.Array)
        {
            ...
            foreach (var clip in clips.EnumerateArray())
            {
                ...
                if (clip.TryGetProperty("duration_seconds", out var durationSeconds) && durationSeconds.ValueKind == JsonValueKind.Number)
                    timing.Add($"duración {durationSeconds.GetDouble():0.#}s");
                var transition = GetFirstString(clip, "transition");
                if (!string.IsNullOrWhiteSpace(transition))
                    timing.Add($"transición {transition}");
