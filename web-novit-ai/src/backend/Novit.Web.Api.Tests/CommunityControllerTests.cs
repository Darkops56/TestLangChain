...
public class CommunityControllerTests
{
    ...
    public async Task TriggerPublishOnDemand_QueuesDraftForReview_WithEditorialOverrides()
    {
        ...
        var agentClient = new CapturingAgentClient
        {
            DraftKickResult = new AgentCommunityDraftKickResult(
                ...
                Topic: "Nueva regulación para importadores",
                Angle: "Qué cambia operativamente para una pyme",
                Objective: "Aprovechar noticia sectorial para abrir conversación comercial")
        };
        ...
        Assert.Equal("Nueva regulación para importadores", agentClient.Topic);
        Assert.Equal("Qué cambia operativamente para una pyme", agentClient.Angle);
        Assert.Equal("Aprovechar noticia sectorial para abrir conversación comercial", agentClient.Objective);
    ...
    public async Task GetTriggerPublishStatus_ReturnsDraftMetadata_WhenJobCompleted()
    {
        ...
        var draft = new CommunityPublicationEntity
        {
            ...
            ReviewSubject = "[REVISIÓN] Tema revisado",
            ReviewToken = "rvw123",
        };
