...
- **BREAKING**: Ambos proyectos NO comparten código, carpetas, librerías ni dependencias. Pueden tener duplicación de código (ej: ruteo a endpoints de LLM) pero cada uno en su propio entorno
- **BREAKING**: El proyecto actual (`TestLangChain`) se convierte en `chatbot-service/` (se renombra)
- **NUEVO**: Crear `cm-platform/` con `cm-backend/` (API REST .NET) y `cm-frontend/` (SPA React)
- **NUEVO**: Crear `cm-frontend/` con interfaz web para el gestor de comunidades
...
- `cm-frontend`: Interfaz web (React) para que el gestor de comunidades administre clientes, revise timelines, apruebe contenido y gestione facturación
...
- Se crea `cm-platform/` como repositorio nuevo con 3 sub-proyectos: `cm-backend/` (API .NET), `cm-agent/` (worker Python), `cm-frontend/` (SPA React)
- NO existe `shared-core/` — cada proyecto repite lo que necesita (duplicación aceptable)
