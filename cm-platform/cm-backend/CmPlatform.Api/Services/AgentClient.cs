...
public sealed class AgentClient : IAgentClient
{
    ...
    public async Task<AgentNewsletterResult?> GenerateNewsletterAsync(CancellationToken ct = default)
    {
        try
        {
            _logger.LogInformation("Requesting newsletter generation from Python agentâ€¦");
            var response = await _http.PostAsync("internal/nurturing/generate-sync", null, ct);

            if (!response.IsSuccessStatusCode)
            {
                var body = await response.Content.ReadAsStringAsync(ct);
                _logger.LogWarning("Agent newsletter generation failed: {Status} â€” {Body}", response.StatusCode, body);
                return null;
            }
        ...
        string currentSubject, string currentBody, string reviewerFeedback, CancellationToken ct = default)
    {
        try
        {
            _logger.LogInformation("Requesting newsletter revision from Python agentâ€¦");
            ...
            {
                var body = await response.Content.ReadAsStringAsync(ct);
                _logger.LogWarning("Agent newsletter revision failed: {Status} â€” {Body}", response.StatusCode, body);
                return null;
            }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            {
                var body = await response.Content.ReadAsStringAsync(ct);
                _logger.LogWarning("Agent community draft revision failed: {Status} â€” {Body}", response.StatusCode, body);
                return null;
            }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            {
                var body = await response.Content.ReadAsStringAsync(ct);
                _logger.LogWarning("Agent community draft generation failed: {Status} â€” {Body}", response.StatusCode, body);
                return null;
            }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            {
                var body = await response.Content.ReadAsStringAsync(ct);
                _logger.LogWarning("Agent community generate-draft-now failed: {Status} â€” {Body}", response.StatusCode, body);
                return null;
            }
    ...
    public async Task<AgentCommunityDraftJobStatusResult?> GetCommunityDraftJobStatusAsync(string jobId, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                var body = await response.Content.ReadAsStringAsync(ct);
                _logger.LogWarning("Agent community draft job status failed: {Status} â€” {Body}", response.StatusCode, body);
                return null;
            }
    ...
    public async Task<bool> DeleteCommunityDraftMediaAsync(string designJson, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                var body = await response.Content.ReadAsStringAsync(ct);
                _logger.LogWarning("Agent community draft media cleanup failed: {Status} â€” {Body}", response.StatusCode, body);
                return false;
            }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            {
                var body = await response.Content.ReadAsStringAsync(ct);
                _logger.LogWarning("Agent community draft media materialization failed: {Status} â€” {Body}", response.StatusCode, body);
                return null;
            }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            {
                var body = await response.Content.ReadAsStringAsync(ct);
                _logger.LogWarning("Agent community draft publish failed: {Status} â€” {Body}", response.StatusCode, body);
                return null;
            }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            {
                var body = await response.Content.ReadAsStringAsync(ct);
                _logger.LogWarning("Agent community publish-now failed: {Status} â€” {Body}", response.StatusCode, body);
                return null;
            }
