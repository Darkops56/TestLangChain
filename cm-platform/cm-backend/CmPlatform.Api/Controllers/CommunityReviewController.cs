    ...
    ILogger<CommunityReviewController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> Approve(string reviewToken, CancellationToken ct)
    {
        ...
        if (draft is null)
            return HtmlPage("Link invÃ¡lido", "No encontramos una publicaciÃ³n asociada a este link de revisiÃ³n.");
        ...
        if (draft.Status == CommunityPublicationStatus.Published)
            return HtmlPage("PublicaciÃ³n ya enviada", $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> ya habÃ­a sido publicada.", success: true);

        if (draft.Status == CommunityPublicationStatus.Denied)
            return HtmlPage("PublicaciÃ³n denegada", $"La propuesta <b>{WebUtility.HtmlEncode(draft.Topic)}</b> fue denegada internamente antes de enviarse a revisiÃ³n. RevisÃ¡ el historial en el panel o generÃ¡ una nueva versiÃ³n.");

        if (draft.Status == CommunityPublicationStatus.Rejected)
            return HtmlPage("PublicaciÃ³n rechazada", $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> ya estaba rechazada y no se publicarÃ¡.");

        if (draft.Status == CommunityPublicationStatus.RevisionQueued)
            return HtmlPage(
                "RevisiÃ³n en proceso",
                $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> ya tiene otra revisiÃ³n en curso. Cuando termine, vas a recibir otro mail para aprobarla o rechazarla.");

        if (draft.Status == CommunityPublicationStatus.MediaGenerationQueued)
            return HtmlPage(
                "GeneraciÃ³n multimedia en proceso",
                $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> ya fue aprobada en la etapa previa y la multimedia se estÃ¡ generando. Cuando la versiÃ³n final estÃ© lista, va a llegar otro mail de revisiÃ³n.");
        ...
        if (draft.Status == CommunityPublicationStatus.PendingReview)
        {
            if (reviewStage == CommunityReviewStage.PreMedia)
            {
                ...
                return HtmlPage(
                    "Plan aprobado",
                    $"Aprobaste el plan de la publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b>. Ahora quedÃ³ en cola la generaciÃ³n de imÃ¡genes o video finales. Cuando esa versiÃ³n estÃ© lista, vas a recibir otro mail para la aprobaciÃ³n final.",
                    success: true);
            }
        ...
        if (publishResult?.InstagramSuccess == true)
        {
            return HtmlPage(
                ...
                $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> fue aprobada y publicada en Instagram correctamente.",
                success: true);
        }
        ...
        return HtmlPage(
            "Aprobada",
            $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> quedÃ³ aprobada, pero la publicaciÃ³n automÃ¡tica no terminÃ³ correctamente. El equipo puede reintentar sin perder la aprobaciÃ³n.");
    }
    ...
    public async Task<IActionResult> RejectForm(string reviewToken, CancellationToken ct)
    {
        ...
        if (draft.Status == CommunityPublicationStatus.Published)
            return HtmlPage("PublicaciÃ³n ya enviada", $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> ya fue publicada y no puede rechazarse desde este link.");

        if (draft.Status == CommunityPublicationStatus.Denied)
            return HtmlPage("PublicaciÃ³n denegada", $"La propuesta <b>{WebUtility.HtmlEncode(draft.Topic)}</b> fue denegada internamente antes de enviarse a revisiÃ³n y no puede rechazarse desde este link.");

        if (draft.Status == CommunityPublicationStatus.Rejected)
            return HtmlPage("PublicaciÃ³n ya rechazada", $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> ya estaba rechazada.");

        if (draft.Status == CommunityPublicationStatus.RevisionQueued)
            return HtmlPage(
                "RevisiÃ³n en proceso",
                $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> ya tiene otra revisiÃ³n en curso con feedback previo. Cuando la nueva versiÃ³n estÃ© lista, vas a recibir otro mail.");

        if (draft.Status == CommunityPublicationStatus.MediaGenerationQueued)
            return HtmlPage(
                "GeneraciÃ³n multimedia en proceso",
                $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> ya estÃ¡ generando la multimedia final. Cuando termine, vas a recibir otro mail para la aprobaciÃ³n final.");
        ...
        CancellationToken ct)
    {
        ...
        if (draft.Status == CommunityPublicationStatus.Denied)
            return HtmlPage("PublicaciÃ³n denegada", $"La propuesta <b>{WebUtility.HtmlEncode(draft.Topic)}</b> fue denegada internamente antes de enviarse a revisiÃ³n y no acepta feedback desde este link.");
        ...
        if (draft.Status == CommunityPublicationStatus.RevisionQueued)
            return HtmlPage(
                "RevisiÃ³n en proceso",
                $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> ya tiene otra revisiÃ³n en curso con feedback previo. EsperÃ¡ la prÃ³xima versiÃ³n por mail.");

        if (draft.Status == CommunityPublicationStatus.MediaGenerationQueued)
            return HtmlPage(
                "GeneraciÃ³n multimedia en proceso",
                $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> ya estÃ¡ generando su versiÃ³n final. Cuando termine, vas a recibir otro mail para la aprobaciÃ³n final.");
        ...
        if (wantsRevision)
        {
            ...
            return HtmlPage(
                "RevisiÃ³n en cola",
                $"Tu feedback fue guardado. La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> ya tiene una revisiÃ³n en curso y vas a recibir la prÃ³xima versiÃ³n por mail cuando salga.",
                success: true);
        }
        ...
        return HtmlPage(
            ...
            $"La publicaciÃ³n <b>{WebUtility.HtmlEncode(draft.Topic)}</b> quedÃ³ rechazada.");
    }
...
<head>
  ...
  <title>Rechazar publicaciÃ³n</title>
  <style>
...
<body>
  <main>
    <h1>Rechazar publicaciÃ³n</h1>
    <p>PublicaciÃ³n: <b>{{encodedTopic}}</b></p>
    <form method="POST">
      <label for="feedback">Feedback (opcional):</label>
            <textarea id="feedback" name="feedback" placeholder="EscribÃ­ los cambios que querÃ©s. Si este campo tiene texto, siempre se tomarÃ¡ como pedido de revisiÃ³n y recibirÃ¡s una nueva versiÃ³n por mail..."></textarea>
            <p class="hint">Si escribÃ­s feedback, siempre se pedirÃ¡ revisiÃ³n y te llegarÃ¡ una nueva propuesta. Para rechazarla definitivamente, dejÃ¡ este campo vacÃ­o.</p>
      <div class="actions">
